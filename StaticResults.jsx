...
import { savePatientBatch, saveCalculationBatch } from '../lib/api';
...
export default function StaticResults() {
  ...
  const patientRows = data.map(p => ({ 
    ...p, 
    source: 'static', 
    hbv: p.hbv === 'Yes' || p.hbv === true, 
    hcv: p.hcv === 'Yes' || p.hcv === true, 
    age: Number(p.age) || null, 
    bilirubin: Number(p.bilirubin) || null, 
    alt: Number(p.alt) || null, 
    ast: Number(p.ast) || null, 
    platelets: Number(p.platelets) || null, 
    bmi: Number(p.bmi) || null 
  }));
  const saved = await savePatientBatch({ patients: patientRows });
  const calcRows = [];
  saved.patients.forEach((pt, idx) => {
    const scores = byPatient[idx]?.scores || {};
    TECHNIQUES.forEach(tech => {
      calcRows.push({ 
        patient_id: pt.id, 
        technique_id: tech.id, 
        risk_result: scores[tech.name] ?? 0, 
        technique_name: tech.name 
      });
    });
  });
  if (calcRows.length) await saveCalculationBatch({ calculations: calcRows });
} catch(e) { 
  console.warn('Static DB save failed:', e.message); 
}
